...
│   └── photos/           ★ 商品写真とロゴ
│       ├── logo.png          ロゴ（大・About セクション用）
│       ├── logo-mark.png     ロゴ（小・ヘッダーとフッター用）
│       └── collection.jpg    About セクションの写真
└── README.md
```
...
| `category` | `ブレスレット` `イヤリング・ピアス` `イヤーカフ` のいずれか |
| `price` | 税込価格（数字のみ） |
...
`assets/photos/logo.png`（480px）と `logo-mark.png`（128px）の2枚を使っています。
どちらも円の外側が透明な PNG なので、明るい画面でも暗い画面でもそのまま置けます。
新しいロゴにするときは、同じサイズの正方形・円形に切り抜いた PNG を同じ名前で上書きしてください。
